...
import vocab
from local_feature import CCMPredEncoder
...
class Dataset(object):
    def __init__(self,
            ...
            fasta=None, local=None,
            use_loc_feat=True, use_glob_feat=True, use_aux_feat=False,
        ...
        self.fasta = fasta
        self.use_aux_feat = use_aux_feat
        ...
        if self.use_loc_feat:
            self.ccmpred_encoder = CCMPredEncoder(
                ccmpred_output=local, seq_len=len(self.native_sequence))
    ...
    def build_data(self, mode, return_df=False):
        ...
        if self.use_aux_feat:
            selected_columns = df[['Tranception_L', 'ESM1v_ensemble', 'MSA_Transformer_ensemble',
                                   'DeepSequence_ensemble', 'TranceptEVE_L']].values
            aux_feat = torch.from_numpy(selected_columns.astype(np.float32))
